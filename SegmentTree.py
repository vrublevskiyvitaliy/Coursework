...
class SegmentTree(object):
    ...
    def query_max_index(self, start, end):
        return self._query_max_index(start, end, self.start, self.end)
    ...
    def _init(self, start, end):
        ...
        self.max_index[(start, end)] = start
        if start < end:
    ...
    def _add(self, start, end, weight, in_start, in_end):
        ...
        if self.max_value[(in_start, mid)] >= self.max_value[(mid+1, in_end)]:
            self.max_index[key] = self.max_index[(in_start, mid)]
        else:
            self.max_index[key] = self.max_index[(mid+1, in_end)]
    ...
    def _query_max_index(self, start, end, in_start, in_end):
        if start == in_start and end == in_end:
            ans = self.max_index[(start, end)]
        else:
            mid = in_start + int((in_end - in_start) / 2)
            if mid >= end:
                ans = self._query_max_index(start, end, in_start, mid)
            elif mid+1 <= start:
                ans = self._query_max_index(start, end, mid+1, in_end)
            else:
                a1 = self._query_max_index(start, mid, in_start, mid)
                a2 = self._query_max_index(mid+1, end, mid+1, in_end)

                if self.max_value[(a1, a1)] >= self.max_value[(a2, a2)]:
                    ans = a1
                else:
                    ans = a2
